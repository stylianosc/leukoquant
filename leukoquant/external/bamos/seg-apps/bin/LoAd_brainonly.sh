...
if [ ! -f ${name}__cpp.nii ]; then
  ...
  nonLinearArgs=" -ln 3 -lp 2 -sx 5 -target ${image} -tmask ${name}__mask_filled.nii -source ${atlas} -aff ${name}__affine.mat -cpp ${name}__cpp.nii -result ${name}__f3d_result.nii.gz"

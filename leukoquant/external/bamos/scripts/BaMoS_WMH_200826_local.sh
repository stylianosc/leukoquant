...
PathReg=/leukoquant/leukoquant/external/niftyreg/gpu/bin
...
PathSeg=/leukoquant/leukoquant/external/bamos/bin
# Standard NiftySeg tools (unmodified from mainline) come from the separately
# built latest NiftySeg, not from bamos/bin/ -- that directory now only holds
# the two BaMoS-specific applications (Seg_BiASM, Seg_Analysis) that don't
# exist in mainline NiftySeg at all.
PathNiftySeg=/leukoquant/leukoquant/external/niftyseg/bin
...
if [ $# -lt 5 ] || [ $# -gt 11 ]; then
	...
	echo "platf: (optional) NiftyReg platform - 0 for CPU (default), 1 for CUDA"
	echo "*******************************************************************************"
	echo ""
	exit
fi
...
seg_maths_bin=${PathNiftySeg}/seg_maths
...
if ((${Space} > 1)); then
	...
	echo "${reg_aladin_bin} -ref ${flair_file} -flo ${ImageT1} -aff ${aff_t1_to_flair} -res ${t1_file} -rigOnly -platf ${PLATF}" >>${NameScript}
	...
	for p in Segmentation prior TIV Parcellation; do
		...
		echo "${reg_resample_bin} -inter 1 -flo ${temp_p_file} -res ${temp_gif_file} -ref ${flair_file} -aff ${aff_t1_to_flair} -platf ${PLATF}" >>${NameScript}
	done
	...
	echo "${reg_aladin_bin} -ref ${t1_file} -flo ${ImageFLAIR} -aff ${aff_flair_to_t1} -res ${flair_file} -rigOnly -platf ${PLATF}" >>${NameScript}
	echo "${seg_maths_bin} ${flair_file} -odt float ${flair_file}" >>${NameScript}
	for p in Segmentation prior TIV Parcellation; do
...
if [ "$JUMP_START" -eq 0 ]; then
	...
	echo "${reg_aladin_bin} -ref ${t1_file} -flo ${PathICBM}/ICBM_Template.nii.gz -aff ${aff_transform_file} -res ${icbm_aladin_file} -platf ${PLATF}" >>${NameScript}
	echo "${reg_f3d_bin} -ref ${t1_file} -flo ${PathICBM}/ICBM_Template.nii.gz -aff ${aff_transform_file} -cpp ${cpp_file} -res ${icbm_f3d_file} -platf ${PLATF}" >>${NameScript}

    cpp_file_pattern=(${PathScratch}/*cpp*)
	echo "cp ${cpp_file_pattern[0]} ${PathResults}/." >>${NameScript}

	for p in CGM DGM ECSF ICSF Out WM; do
		echo "${reg_resample_bin} -ref ${t1_file} -flo ${PathICBM}/ICBM_${p}.nii.gz -cpp ${cpp_file} -res ${PathScratch}/ICBM_${p}_${ID}.nii.gz -platf ${PLATF}" >>${NameScript}
	done

...
seg_EM::seg_EM(int _numb_classes, int _nu,int _nt) // Constructor for all parameters and values needed to give a segmentation result
{
  ...
  this->CurrSizes=NULL;
  this->reg_factor=1.1f;// whereas in common set to 2
...
int seg_EM::SetInputImage(nifti_image *r)
{
  ...
  // Size
    // measures how many dimensions are effectively active. Need an (int) cast to transform from boolean for each test
  ...
  this->dy=r->dy;
  this->dz=r->dz;
  this->numel=r->nz*r->ny*r->nx;// Number of voxels
...
int seg_EM::SetPriorImage(nifti_image *r)
{
  ...
  if(this->nx==r->nx && this->ny==r->ny && this->nz==r->nz){
      /* check that the 3D dimensions are the same for the this segmentation result (that must be of
...
int seg_EM::SetMaskImage(nifti_image *f)
{
  ...
  this->numelmasked=0;
  // Determination of the number of active voxels
  for(int i=0; i<this->numel; i++,MaskDataPtr++){
      if((*MaskDataPtr)>0){
          this->numelmasked++;
        }
    }
  if(this->nx==f->nx && this->ny==f->ny && this->nz==f->nz){// Check for the appropriate dimensions (for 3D volume)
      return 0;
    }
  else{
...
// Define verbose level (mostly corrects for user's errors because can only take 0 1 and 2 as values)
...
int seg_EM::Allocate_and_Initialize()
{
  ...
  if(this->Priors_status){// case we are using priors
      if(this->maskImage_status){
...
int seg_EM::Intensity_Based_Inisitalization_of_Means()
{
  ...
  for(int ms=0; ms<this->nu; ms++){// ms for multispectral
      ...
      meanval=meanval/mycounter; // mean of the image over the mask
...
// Determination of max and min of each image
      for(int i=0; i<this->numel; i++){
          if(!this->maskImage_status || MaskDataPtr[i]>0){

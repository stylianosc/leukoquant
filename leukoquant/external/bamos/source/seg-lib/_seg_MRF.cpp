...
// Updating of MRFpriors when input corresponds to active voxels
...
#pragma omp parallel for shared(Expec,MRFprior,Long_2_Short_IndicesPtr)
#endif
      for (int iz=1; iz<maxiz-1; iz++) {// Do not take borders into account
          ...
          for (int iy=1; iy<maxiy-1; iy++) {
              ...
              for (int ix=1; ix<maxix-1; ix++) {
                  ...
                  if (curr_short_centreindex>=0) { // if indexCentre is indeed an active voxel, definition of the indices of the neighbors
                      ...
                      for (currclass=0; currclass<numclass; currclass++){
                          ...
                          if(currclass<numclass){ //incrementation of class : look numelmasked voxels further away
                      ...
                      for (currclass=0; currclass<numclass; currclass++) { // Normalisation step
  ...
  else{ // If no MRF use, just input the Atlas Priors
...
// Same as before when no mask is considered, therefore no need of L2S
                       ...
                       int verbose_level)
{
  ...
  if(MRFflag){
      ...
      //unsigned int image_size_currclass_shift[max_numbclass];
      for(int i=0; i<numclass; i++){ // In contrast to previous function, the shift is of numel and not only numelmasked
      ...
      for (iz=0; iz<maxiz-0; iz++) {
          for (iy=0; iy<maxiy-0; iy++) {
              for (ix=0; ix<maxix-0; ix++) {
                  Sum_Temp_MRF_Class_Expect = 0;
                  // Definition of neighbouring voxels. Not same way of handling borders...
                  ...
                  for (currclass=0; currclass<numclass; currclass++){
                      ...
                      Hplane[currclass] = 0.0;
                      Temp_MRF_Class_Expect[currclass] = 0.0;
                      Gplane[currclass]+=((indexWest>=0)?Expec[indexWest]:0); // Set to 0 if at the border

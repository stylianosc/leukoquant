...
if [ ! -f ${name}__cpp.nii ]; then
  ...
  ################################################################
  reg_resample -target ${image} -source $grey_prior -cpp ${name}__cpp.nii -result ${name}__registered_grey.nii -TRI -platf 0
  reg_resample -target ${image} -source $white_prior -cpp ${name}__cpp.nii -result ${name}__registered_white.nii -TRI -platf 0
  reg_resample -target ${image} -source $csf_prior -cpp ${name}__cpp.nii -result ${name}__registered_csf.nii -TRI -platf 0
  reg_resample -target ${image} -source $deep_grey_prior -cpp ${name}__cpp.nii -result ${name}__registered_deep_grey.nii -TRI -platf 0
  reg_resample -target ${image} -source $internal_csf_prior -cpp ${name}__cpp.nii -result ${name}__registered_internal_csf.nii -TRI -platf 0

...
/* Merge all separate nifti images used for the priors into a single image with all classes on the fourth dimension
 *Priors_temp is an array of nifti images. The number of priors used is contained in segment_param. All data are cast
 *to SegPrecisionType chosen (that is float)
*/
// ????? Why not change dim[0]=4 instead of 3 and dim[4]=segment_param->numb_classes ?
...
int main(int argc, char **argv)
{
  ...
  for(int i=1;i<argc;i++){
      if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 ||
         ...
         strcmp(argv[i], "--h")==0 || strcmp(argv[i], "--help")==0){
          ...
          segment_param->OutliernessThreshold=atof(argv[++i]);// atof converts string to double
  ...
  else if(InputImage->nt>1){ // ????? why exactly same condition as before ?? why not <1 instead ??
      // why not possible to have both nu and nt more than 1
      InputImage->dim[5]=1;
      InputImage->dim[4]=1;
    }
  InputImage->dim[0]=5;
  ...
  // Allocation of memory for the array of priors images

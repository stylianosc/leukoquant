...
rapidxml::xml_attribute<> * xmlAddAtributeToItem(rapidxml::xml_document<> * baseDocument, rapidxml::xml_node<> * node, std::string nameString, std::string valueString=""){
  // [leukoquant patch] length() excludes the null terminator that strcpy()
  // still writes -- these allocations were 1 byte short of what strcpy()
  // actually needs, overflowing the heap buffer by exactly 1 byte on every
  // call (confirmed via AddressSanitizer: heap-buffer-overflow,
  // WRITE of size 9 into an 8-byte region, discovered 2026-08-26 while
  // GIF was writing its results XML report at the very end of a run --
  // after the real image outputs had already saved successfully).
  char * valueChar= new char[valueString.length() + 1];
  strcpy(valueChar,valueString.c_str());
  char * nameChar= new char[nameString.length() + 1];
...
rapidxml::xml_node<> * xmlAddNodeToItem(rapidxml::xml_document<> * baseDocument, rapidxml::xml_node<> * node, std::string nameString, std::string valueString=""){
  // [leukoquant patch] Same off-by-one as xmlAddAtributeToItem() above --
  // length() excludes strcpy()'s null terminator.
  char * valueChar= new char[valueString.length() + 1];
  strcpy(valueChar,valueString.c_str());
  char * nameChar= new char[nameString.length() + 1];
  strcpy(nameChar,nameString.c_str());
...
void GIF::set_regPlatf(int _value)
{
    // 0=CPU (default), 1=CUDA. Requires seg_GIF to have been built against
    // a CUDA-enabled NiftyReg; ignored (falls back to CPU) otherwise.
    this->regPlatf=_value;
}

void GIF::set_probthresh(float _value)
{
    this->probthresh=_value;
}
...
void GIF::processAndSaveFlowAsLabelSeg(unsigned int databasenumb)
{
    ...
    for(size_t index=0; index<MaskNii->nvox; index++)
    {
        if(this->targetMask[index])
        {
            ...
            else
            {
                ...
                MaskNiiPtr[index]= ( (MaskNiiPtr[index] / normalized_weight) > this->probthresh )?1:0;
            }
   ...
   // If using Update
    ...
    // [leukoquant patch] Restored from upstream commit d6f47e2 ("Remove
    // Brain/TIV output", jcardoso@cmiclab, 2017-08-23), which deleted this
    // block from the private KCL-BMEIS/gif_extras fork. Public
    // KCL-BMEIS/gif's "main" branch never had this at all -- it descends
    // from a point after the removal. This pipeline's existing production
    // GIF binary (leukoquant/external/gif/bin/seg_GIF, built pre-removal)
    // does still produce these two files, and downstream consumers expect
    // them (confirmed empirically: a real DPUK subject's historical GIF
    // output includes both _Brain.nii.gz and _TIV.nii.gz), so restoring
    // this exact original block keeps output parity with the existing
    // production binary instead of silently dropping two files.

    // Save brain mask (all vox with label>=2)
    smartPointer<niftiObject> BrainMask = new niftiObject(nifti_copy_nim_info(this->target->getPtr()));
    nifti_image *BrainMaskPtr = BrainMask->getPtr();
    BrainMaskPtr->dim[0]=BrainMaskPtr->ndim=3;
    BrainMaskPtr->dim[4]=BrainMaskPtr->nt=1;
    BrainMaskPtr->dim[5]=BrainMaskPtr->nu=1;
    nifti_update_dims_from_array(BrainMaskPtr);
    BrainMaskPtr->datatype = NIFTI_TYPE_FLOAT32;
    BrainMaskPtr->nbyper = sizeof(float);
    BrainMaskPtr->data = (void *)malloc(BrainMaskPtr->nvox * BrainMaskPtr->nbyper);
    float * BrainMaskDataPtr=static_cast<float *>(BrainMaskPtr->data);
    for(int index=0; index<ResultingLabels->nx*ResultingLabels->ny*ResultingLabels->nz; index++)
        BrainMaskDataPtr[index]=(ResultingLabelsPtr[index]>=2)?1:0;
    filenameStr=this->resultsPath+SEP+strippedTargetFileName+"_"+this->infoDB[databasenumb]->getFname()+"_Brain.nii.gz";
    nifti_set_filenames(BrainMaskPtr,filenameStr.c_str(),0,0);
    BrainMaskPtr->cal_max=1;
    BrainMaskPtr->cal_min=0;
    nifti_image_write(BrainMaskPtr);


    // Save TIV mask (all vox with label>=1)
    smartPointer<niftiObject> TIVmask = new niftiObject(nifti_copy_nim_info(this->target->getPtr()));
    nifti_image *TIVmaskPtr = TIVmask->getPtr();
    TIVmaskPtr->dim[0]=TIVmaskPtr->ndim=3;
    TIVmaskPtr->dim[4]=TIVmaskPtr->nt=1;
    TIVmaskPtr->dim[5]=TIVmaskPtr->nu=1;
    nifti_update_dims_from_array(TIVmaskPtr);
    TIVmaskPtr->datatype = NIFTI_TYPE_FLOAT32;
    TIVmaskPtr->nbyper = sizeof(float);
    TIVmaskPtr->data = (void *)malloc(TIVmaskPtr->nvox * TIVmaskPtr->nbyper);
    float * TIVmaskDataPtr=static_cast<float *>(TIVmaskPtr->data);
    for(int index=0; index<ResultingLabels->nx*ResultingLabels->ny*ResultingLabels->nz; index++)
        TIVmaskDataPtr[index]=(ResultingLabelsPtr[index]>=1)?1:0;
    filenameStr=this->resultsPath+SEP+strippedTargetFileName+"_"+this->infoDB[databasenumb]->getFname()+"_TIV.nii.gz";
    nifti_set_filenames(TIVmaskPtr,filenameStr.c_str(),0,0);
    TIVmaskPtr->cal_max=1;
    TIVmaskPtr->cal_min=0;
    nifti_image_write(TIVmaskPtr);
...
bool GIF::postprocessXML(string saveFilename, string baseXMLFilename, vector< vector<int> > mapping, nifti_image * BiasFieldCorrected, nifti_image * TissueSegmentation, nifti_image * categoricalSegmentation, mapImage * currentInfoDB)
{
    ...
    if(strcmp(base_node->name(),"tissues")==0)
    {
      for (rapidxml::xml_node<> *current_node=base_node->first_node(); current_node; current_node=current_node->next_sibling())
      {
        if(strcmp(current_node->name(),"item")==0)
        {
          bool exists=false;
          char * endptr;
          // [leukoquant patch] xmlGetTextFromXmlNode() returns a std::string
          // by value; taking .c_str() straight off that temporary and only
          // storing the char* means the temporary (and its buffer) is
          // destroyed at the end of this statement, before strtol() below
          // reads through inptr -- a dangling pointer (confirmed via
          // AddressSanitizer: stack-use-after-scope, discovered 2026-08-26).
          // Every other call site in this function avoids this by storing
          // the std::string itself first (see the <labels>/<snr>/<cnr>
          // blocks above/below), which is the fix applied here too.
          string instr=xmlGetTextFromXmlNode(current_node,(char*)"number",&exists);
          const char *inptr=instr.c_str();
  ...
  // [leukoquant patch] Upstream bug: this function is declared 'bool' but had no
  // return statement on the normal/success path -- confirmed by GCC's own
  // "control reaches end of non-void function" warning at this exact site.
  // Falling off the end of a non-void function is undefined behavior in C++;
  // GCC is known to exploit such UB non-locally within the same function under
  // optimization (it may treat code paths as unreachable and eliminate or
  // reorder unrelated checks elsewhere), which is the leading suspect for a
  // reproducible SIGSEGV (null-pointer call to rapidxml::xml_node::next_sibling())
  // seen at the very end of this function, after all of its real work -- including
  // writing this XML file -- had already completed successfully.
  return true;
}
...
nifti_image * GIF::createDistanceLNCC(unsigned int subnumb, nifti_image * Deformation)
{
    ...
    // [leukoquant patch] malloc(), not new[] -- Results becomes
    // ResampledFloating->data below (permanently, not just scratch), which
    // gets freed via nifti_image_free()'s plain free() call whenever this
    // returned image is later destroyed. new[]/free() is a real allocator
    // mismatch (confirmed via AddressSanitizer: alloc-dealloc-mismatch,
    // operator new[] vs free, right here) -- malloc() matches what
    // nifti_image_free() actually expects, same convention already used a
    // few lines below for the ResampledFloating->data fallback allocation.
    float * Results=(float *)malloc(volSize * sizeof(float));
...
nifti_image * GIF::createDistanceT1DTI(unsigned int subnumb, nifti_image * Deformation)
{
    ...
    float * InputImageStd=new float [volSize];
    float * ReferenceInputCovar=new float [volSize];
    // [leukoquant patch] malloc(), not new[] -- same reasoning as Results in
    // createDistanceLNCC() above: ResultsT1 becomes ResampledFloating->data
    // permanently, freed later via nifti_image_free()'s free().
    float * ResultsT1=(float *)malloc(volSize * sizeof(float));
...
//#endif
    ...
    #pragma omp critical
    {
            ...
            REG_AFFINE->SetPlatformType(this->regPlatf==1 ? PlatformType::Cuda : PlatformType::Cpu);
            ...
            const mat44 * affine_tmp=REG_AFFINE->GetTransformationMatrix();
...
//    #endif
    ...
    #pragma omp critical
    {
            ...
            REG_F3D->SetPlatformType(this->regPlatf==1 ? PlatformType::Cuda : PlatformType::Cpu);
            ...
            // [leukoquant patch] GetControlPointPositionImage() used to return a
            // plain caller-owned nifti_image*, which _own=true (below) correctly
            // took ownership of. The current vendored NiftyReg wraps it in
            // RNifti's reference-counted NiftiImage instead: the temporary
            // returned here gets destroyed (freeing the underlying data) right
            // after this statement, regardless of what _own claims, since RNifti
            // owns the real lifetime now. _own=false makes niftiDynamicObject
            // deep-copy the data instead (its own existing behavior for that
            // flag) while the temporary is still alive, avoiding the
            // use-after-free that was crashing GIF later in the pipeline
            // ("Similarity" stage) when it tried to read back the freed cpp data.
            smartPointer<niftiDynamicObject> resultCPP=new niftiDynamicObject(REG_F3D->GetControlPointPositionImage(),false);
            delete REG_F3D;

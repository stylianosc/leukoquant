...
// [leukoquant patch] Upstream guards this file with '#pragma once' only, which
// prevents re-inclusion of this exact physical file but not of NiftySeg's
// content-identical vendored copy of the same header (same collision the
// pre-refactor vendored NiftyReg had, now reproduced against the current
// upstream source too). A translation unit that links against both
// NiftyReg and NiftySeg (e.g. GIF's _seg_GIF.cpp) would otherwise get struct
// redefinition errors. Add the matching classic named guard NiftySeg already
// uses so the two copies correctly deduplicate against each other.
//
// RNIFTI_NIFTILIB_VERSION must be defined unconditionally, ABOVE the
// _NIFTI_IO_HEADER_ guard, not inside it: when NiftySeg's copy of this file
// (which doesn't know about RNIFTI_NIFTILIB_VERSION at all) is included
// first in a translation unit that links both libraries, it claims the
// shared guard name first, and this entire file's real content -- including
// the definition below -- would otherwise be silently skipped. Without it,
// NiftiImage.h's `#if RNIFTI_NIFTILIB_VERSION == 1/2` type-alias blocks
// (dim_t, pixdim_t, scale_t, etc.) both evaluate false and those types
// never get declared.

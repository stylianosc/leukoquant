...
double reg_f3d2<T>::ComputeJacobianBasedPenaltyTerm(int type) {
    ...
    if (type > 0 && it > 0) {
        if (backwardPenaltyTerm != backwardPenaltyTerm) {
            // [leukoquant patch] same missing-null-check as the forward-
            // transformation version in _reg_f3d.cpp's own
            // ComputeJacobianBasedPenaltyTerm -- this->optimiser can still
            // be null during the optimiser's own first (initialisation)
            // objective-function evaluation. See that file's patch comment
            // for the full call-chain explanation.
            if (this->optimiser)

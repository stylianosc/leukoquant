...
double reg_f3d<T>::ComputeJacobianBasedPenaltyTerm(int type) {
    ...
    if (type > 0) {
        if (value != value) {
            // [leukoquant patch] this->optimiser can legitimately still be
            // null here: SetOptimiser() builds it via
            // this->optimiser.reset(platform->CreateOptimiser<T>(...)), and
            // CreateOptimiser() calls optimiser->Initialise() (on its own,
            // not-yet-assigned local) before returning -- which evaluates
            // the objective function for the first time, reaching this exact
            // code path, before this->optimiser.reset() has actually run.
            // If that first evaluation folds (NaN, the case here), the
            // unconditional call below null-derefs (confirmed via
            // AddressSanitizer: SEGV on address 0x0, this exact line).
            // "Restore best DOF" is meaningless at this point anyway -- no
            // optimisation step has run yet, so there is no best-DOF history
            // to restore -- skipping it when null is the correct behaviour,

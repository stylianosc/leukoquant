...
public:
    ...
    reg_aladin();
    // [leukoquant patch] reg_aladin has virtual methods (Run() included) and
    // a derived class (reg_aladin_sym) meant to be used polymorphically --
    // e.g. GIF's _seg_GIF.cpp does exactly `reg_aladin<float> *p = new
    // reg_aladin_sym<float>; ... delete p;`. Without a virtual destructor
    // here, that delete only knows the base class's size/layout, corrupting
    // the heap on every call (confirmed via AddressSanitizer:
    // new-delete-type-mismatch, allocated-as-440-bytes/deleted-as-336-bytes)
    // -- this was the root cause of a real, non-deterministic SIGSEGV crash
    // in GIF's CPU registration path, discovered 2026-08-25.
    virtual ~reg_aladin() {}
